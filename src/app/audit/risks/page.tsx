    ...
    <div className="min-h-screen bg-[#0b0f19] text-slate-100 flex flex-col font-sans">
      ...
      <header className="border-b border-slate-800 bg-[#0f172a]/80 backdrop-blur-md sticky top-0 z-50 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight text-white">Maintenance Risk Log</h1>
            <p className="text-[10px] text-emerald-400 font-mono tracking-wider uppercase">FMEA & Mitigation</p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 p-6 max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="p-4 bg-[#0f172a]/40 border border-slate-800 rounded-xl">
                  ...
                  return (
                    <div
                      ...
                      className={`p-5 cursor-pointer hover:bg-slate-900/10 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 ${
                        activeRisk?.id === risk.id ? "bg-slate-900/20 border-l-2 border-emerald-500" : ""
                      }`}
                    >
                      <div className="space-y-1.5 flex-1">
                        <div className="flex items-center gap-3">
                          <span className="font-mono text-xs text-emerald-400 font-semibold">{risk.riskNumber}</span>
                          <span
                        ...
                        <p className="text-slate-200 text-xs font-semibold leading-relaxed">{risk.identifiedRisk}</p>
              ...
              <form onSubmit={handleUpdateMitigation} className="space-y-4">
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between border-b border-slate-800 pb-2">
                    ...
                    <span className="font-semibold text-slate-200">{activeRisk.likelihood} / 5</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-800 pb-2">
                    <span className="text-slate-500">Consequence Impact</span>
                    <span className="font-semibold text-slate-200">{activeRisk.consequence} / 5</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-800 pb-2">
                    <span className="text-slate-500">Status</span>
                    <span className="font-semibold text-slate-200 uppercase">{activeRisk.status}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-400 uppercase">Mitigation Measure Plan</label>
                  <textarea
                    required
                    ...
                    className="w-full h-32 bg-slate-900 border border-slate-800 focus:border-slate-700 rounded-lg p-2.5 text-xs text-slate-200 focus:outline-none resize-none"
                  />
                </div>
