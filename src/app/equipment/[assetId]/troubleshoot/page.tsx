  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-500 font-mono text-xs gap-2">
        <Loader2 className="w-6 h-6 animate-spin text-emerald-600" /> Loading troubleshooting wizard...
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
      ...
      <header className="border-b border-slate-200 bg-white/90 backdrop-blur-md sticky top-0 z-50 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight text-slate-900">Diagnostics Wizard</h1>
      ...
      <main className="flex-1 p-6 max-w-4xl w-full mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="p-5 bg-white border border-slate-200 rounded-xl space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">Select Symptom or Error Code</h3>
            <select
          ...
          {activeGuide ? (
            <div className="p-6 bg-white border border-slate-200 rounded-xl space-y-6">
              <div className="border-b border-slate-200 pb-4">
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wide">Step-by-Step Diagnostic Path</h3>
                <p className="text-xs text-slate-500 mt-1.5">**Probable Cause:** {activeGuide.probableCause}</p>
              </div>
                    ...
                    <div
                      ...
                      className={`p-4 rounded-lg border cursor-pointer transition-all flex items-start gap-3 ${
                        isDone
                          ...
                          : "bg-slate-100 border-slate-200 text-slate-900 hover:border-slate-200"
                      }`}
                    >
                      <div className="mt-0.5">
                        <CheckCircle2 className={`w-4 h-4 ${isDone ? "text-emerald-600" : "text-slate-700"}`} />
                      </div>
              ...
              <div className="space-y-2 pt-4 border-t border-slate-200">
                <label className="text-xs font-semibold text-slate-500 uppercase">Resolution Outcome</label>
                <input
                  type="text"
            ...
            <div className="p-8 bg-slate-100 border border-slate-200 rounded-xl text-center text-slate-500 text-xs">
        ...
        <div className="space-y-6">
          <div className="p-5 bg-white border border-slate-200 rounded-xl space-y-4">
            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wide border-b border-slate-200 pb-3">
              Reference Schematic
            </h3>
            ...
            {activeGuide && activeGuide.componentTag ? (
              <div className="space-y-4 text-xs">
                <div className="p-3 bg-slate-100 border border-slate-200 rounded-lg space-y-2">
                  <p className="font-bold text-emerald-600">Target Component: {activeGuide.componentTag}</p>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between border-b border-slate-200 pb-2">
                    ...
                    <span className="font-semibold text-slate-900">Sheet 6</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-200 pb-2">
                    <span className="text-slate-500">Cabinet Grid Coordinate</span>
                    <span className="font-semibold text-emerald-600">Zone B2</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-200 pb-2">
                    <span className="text-slate-500">Wiring Terminals</span>
                    <span className="font-semibold text-slate-900">Wire 104, Wire 105</span>
                  </div>
                </div>

                {/* Simulated Schematic Sheet Image */}
                <div className="border border-slate-200 rounded-lg overflow-hidden relative bg-white p-2.5 text-center flex flex-col justify-center items-center h-44 border-dashed border-2">
