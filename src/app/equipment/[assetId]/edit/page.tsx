...
export default function EquipmentEditPage() {
  ...
  return (
    <div className="p-6 max-w-3xl w-full mx-auto space-y-8">
      ...
      <form onSubmit={save} className="bg-surface border border-line rounded-xl shadow-card p-6 space-y-5">
        {error && (
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-3">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs text-ink-700">Requires calibration</span>
          </div>
          <div className="flex items-center gap-2.5">
            <Toggle checked={!!form.requiresPremob} onChange={(v) => set("requiresPremob", v)} ariaLabel="Requires pre-mobilization" />
            <span className="text-xs text-ink-700">Requires pre-mobilization (premob)</span>
          </div>
        </div>
